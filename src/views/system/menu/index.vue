    ...
    <div class="main-content">
      ...
      <el-row class="pt20 pb10">
        <el-col :span="24">
          ...
          <el-button type="warning" class="mr20 mb10" :disabled="selectOption.multiple" @click="normalBatchManageMenu">批量启用</el-button>
          <el-button type="warning" class="mr20 mb10" :disabled="selectOption.multiple" @click="stopBatchManageMenu">批量停用</el-button>
      ...
      <el-row class="pb10">
        <el-col :span="24">
          ...
          >
            ...
            <el-table-column prop="status" min-width="180" label="状态"></el-table-column>
            <el-table-column prop="isHiddenDes" min-width="180" label="是否隐藏"></el-table-column>
            ...
            <el-table-column label="操作" fixed="right" width="180">
              <template slot-scope="scope">
                ...
                <el-button v-if="scope.row.status != '0'" size="mini" type="text" class="c-darkBlue" @click="normalManageMenu(scope.row)">启用</el-button>
  ...
  methods: {
    ...
    normalConfirm() {
      if (this.normalForm.type === 1) {
        this.$confirm('此操作将单个启用账号, 是否继续?', '账号启用', {
          ...
          normalManageMenu(this.normalForm).then(res => {
            this.$notify.success({
              title: '操作成功'
            })
            this.getDataList()
        ...
        this.$confirm('此操作将批量启用账号, 是否继续?', '账号启用', {
          ...
          this.dialogNormal.loading = true
          normalBatchManageMenu(this.normalForm).then(res => {
            this.$notify.success({
              title: '操作成功'
            })
            this.getDataList()
    ...
    stopManageMenu(row) {
      this.$confirm('此操作将单个停用账号, 是否继续?', '账号停用', {
        ...
        stopManageMenu({ userId: row.userId }).then(res => {
          this.$notify.success({
            title: '操作成功'
          })
          this.getDataList()
    ...
    stopBatchManageMenu() {
      this.$confirm('此操作将批量停用账号, 是否继续?', '账号停用', {
        ...
        stopBatchManageMenu(this.selectOption.ids).then(res => {
          this.$notify.success({
            title: '操作成功'
          })
          this.getDataList()
